...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::create('user_profile', function (Blueprint $table) {
            $table->increments('id')->comment('ID data profile user');
            $table->foreignId('user_id')->constrained('users')->comment('Relasi ke tabel users');
            $table->string('profile_picture', 255)->comment('Foto Profile atau Pas Foto (氏名)');
            ...
            $table->string('nationality', 100)->comment('Kewarganegaraan (国籍)');
            ...
            $table->date('entry_date')->comment('Tanggal masuk Jepang (入国日)');
            $table->date('visa_expiry_date')->comment('Masa berlaku visa (在留カードの期限)');
            $table->string('current_visa_type', 100)->comment('Jenis visa saat ini (現在の在留資格)');
            $table->enum('jlpt_level', ['N1', 'N2', 'N3', 'N4', 'N5', 'none'])->comment('Level bahasa Jepang (日本語能力)');
            $table->boolean('has_driver_license')->comment('Punya SIM atau enggak (運転免許有無)');
