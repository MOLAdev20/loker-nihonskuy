...
@section('content')
  ...
  <div class="grid grid-cols-1 gap-2 lg:grid-cols-4 lg:gap-5">
    <div
      class="relative rounded-2xl border border-slate-100 bg-white p-6 shadow-sm transition-shadow duration-300 hover:bg-slate-50 hover:shadow-md">
      <div
        class="{{ request('filter') ? 'hidden' : '' }} absolute right-2 top-2 h-3 w-3 animate-ping rounded-full border border-green-500 bg-green-200">
      </div>
    ...
    <div
      class="relative rounded-2xl border border-slate-100 bg-white p-6 shadow-sm transition-shadow duration-300 hover:bg-slate-50 hover:shadow-md">
      <div
        class="{{ request('filter') != 'active' ? 'hidden' : '' }} absolute right-2 top-2 h-3 w-3 animate-ping rounded-full border border-green-500 bg-green-200">
      </div>
      ...
      <p class="mt-1 text-xs text-slate-400">Seluruh lowongan kerja</p>
      <a href="{{ route('admin.vacancies', ['filter' => 'active']) }}"
        class="absolute inset-0 z-10 opacity-0"></a>
    </div>
    <div
      class="relative rounded-2xl border border-slate-100 bg-white p-6 shadow-sm transition-shadow duration-300 hover:bg-slate-50 hover:shadow-md">
      <div
        class="{{ request('filter') != 'inactive' ? 'hidden' : '' }} absolute right-2 top-2 h-3 w-3 animate-ping rounded-full border border-green-500 bg-green-200">
      </div>
      ...
      <p class="mt-1 text-xs text-slate-400">Seluruh lowongan kerja</p>
      <a href="{{ route('admin.vacancies', ['filter' => 'inactive']) }}"
        class="absolute inset-0 z-10 opacity-0"></a>
    </div>
    <div
      class="relative cursor-pointer rounded-2xl border border-slate-100 bg-white p-6 shadow-sm transition-shadow duration-300 hover:bg-slate-50 hover:shadow-md">
      <div
        class="{{ request('filter') != 'on-going-expired' ? 'hidden' : '' }} absolute right-2 top-2 h-3 w-3 animate-ping rounded-full border border-green-500 bg-green-200">
      </div>
      ...
      <a href="{{ route('admin.vacancies', ['filter' => 'on-going-expired']) }}"
        class="absolute inset-0 z-10 opacity-0"></a>
    </div>
  </div>
  ...
  <div class="mt-5 rounded-lg border border-slate-200 p-3">
    <div class="mt-2 grid grid-cols-1 gap-2">
      <form method="GET" action="" class="lg:w-90 relative w-full">
        ...
        <a href="{{ route('admin.vacancies') }}"
          class="{{ !request('q') ? 'hidden' : '' }} absolute inset-y-0 right-3 flex items-center text-slate-400"><x-icons.close /></a>
      </form>
      @forelse ($jobs as $job)
        <div
          ...
          <div class="mt-2 flex flex-wrap gap-1 text-[11px] text-slate-600">
            ...
            <span
              class="rounded bg-slate-100 px-2 py-0.5">{{ $job->gender_requirement == 'p' ? 'Perempuan' : 'Laki-laki' }}</span>
          ...
          <div class="relative z-20 mt-2">
            <a href="{{ route('admin.vacancy.detail', $job->job_code) }}"
              class="text-xs font-medium text-slate-600 hover:text-slate-800">Detail</a>
            <span class="mx-1 text-slate-300">•</span>
            <a href="{{ route('admin.vacancy.edit', $job->job_code) }}"
              class="text-xs font-medium text-slate-600 hover:text-slate-800">Edit</a>
            <span class="mx-1 text-slate-300">•</span>
            <form method="post" id="delete-vacancy-{{ $job->job_code }}"
              action="{{ route('admin.vacancy.delete', $job->job_code) }}" class="hidden">
              @csrf
              @method('DELETE')
            </form>
            <button type="button" onclick="confirmDelete('delete-vacancy-{{ $job->job_code }}')"
              class="cursor-pointer text-xs font-medium text-red-600 hover:text-red-800">Hapus</button>
          </div>
