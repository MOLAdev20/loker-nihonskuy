...
@section('content')
  @php
    ...
    $resolveEducationOption = function (?string $value, array $options) {
        ...
        return $options[$value]['jp'];
    };
    $resolveWorkOption = function (?string $value, array $options) {
        if (! filled($value) || ! isset($options[$value])) {
            return $value ?: '-';
        }
  ...
  <section class="mx-auto w-full max-w-[1240px] px-8 lg:px-16 xl:px-20">
    <div class="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
      <div>
        <h1 class="text-2xl font-semibold tracking-tight text-slate-900">候補者プロフィール</h1>
        ...
        <div class="rounded-xl border border-slate-200 bg-white p-4 shadow-sm sm:p-5">
          ...
          <div class="mt-4 overflow-x-auto rounded-lg border border-slate-200">
            @forelse ($educationHistories as $educationHistory)
              ...
              @endphp
              <tr>
                <td class="px-4 py-3 text-slate-900">{{ $educationLabel }}</td>
          ...
          <div class="mt-4 overflow-x-auto rounded-lg border border-slate-200">
            @forelse ($workExperiences as $workExperience)
              ...
              <tr>
                ...
                <td class="px-4 py-3 text-slate-700">
                  {{ $statusLabel }}{{ $workExperience->visa_type ? ' • ' . $workExperience->visa_type : '' }}
                </td>
              </tr>
              @if ($loop->last)
                  </tbody>
