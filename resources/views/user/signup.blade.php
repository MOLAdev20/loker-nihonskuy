...
<body class="min-h-full bg-slate-100 text-slate-900">
        ...
        <div class="grid w-full max-w-5xl overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-2xl lg:grid-cols-2">
            <section class="hidden bg-slate-900 p-10 text-slate-100 lg:flex lg:flex-col lg:justify-between">
                <div>
                    ...
                    <h1 class="mt-6 text-3xl font-bold leading-tight">Buat akun <span class="text-yellow-400">NihonSkuy</span> dan mulai cari kerja impianmu</h1>
                </div>
                <p class="text-sm text-slate-400">Daftar gratis, cepat, dan aman.</p>
            </section>

            <section class="p-6 sm:p-10">
                <h2 class="text-2xl font-bold">Sign Up</h2>
                <p class="mt-2 text-sm text-slate-500">Isi data kamu untuk mulai melamar pekerjaan.</p>

                <form action="{{ url('sign-up') }}" method="POST" class="mt-8 space-y-5">
                    @csrf
                    ...
                    <div>
                        <label for="password_confirmation" class="mb-2 block text-sm font-medium">Konfirmasi Password</label>
                        <div class="relative">
                            <input
                    ...
                    <p>Sudah punya akun? <a href="{{ url('sign-in') }}" class="text-sm font-semibold text-slate-500 transition hover:text-slate-600">Login di sini</a></p>
                </form>
            </section>
        </div>
    </main>
    ...
    <script>
        ...
        const wireToggle = (buttonSelector, inputId) => {
            ...
            const eyeSlash = toggleButton.querySelector('.icon-eye-slash');
        ...
        wireToggle('[data-toggle-password]', 'password');
        wireToggle('[data-toggle-confirm]', 'password_confirmation');
    </script>
</body>
